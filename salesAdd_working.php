...
const INVOICE_PREFIX = 'JU-';

function generateRandomInvoice(): string {
    $chars = "003232303232023232023456789";
    return INVOICE_PREFIX . substr(str_shuffle($chars), 0, 8);
}
...
try {
    ...
    $conn->rollBack();
    error_log("Database Error in salesAdd.php: " . $e->getMessage());
    echo '<div class="alert alert-danger">An error occurred while processing your request. Please try again.</div>';
    exit;
} catch (Exception $e) {
    $conn->rollBack();
    error_log("General Error in salesAdd.php: " . $e->getMessage());
...
<head>
    <meta charset="UTF-8">
    <title>Periodic Data Entry</title>
...
<div id="register_container" class="sales clearfix">
            ...
            <div class="row forms-area">
                <div class="col-md-8 no-padd">
                    <div class="input-append">
                        <form action="salesAdd.php" method="post" id="add_item_form" class="form-inline" autocomplete="off">
                    ...
                    <table id="register" class="table table-bordered">
                        ...
                        <?php foreach ($saveDetails as $row): ?>
                            <tr id="reg_item_top" bgcolor="#eeeeee">
                                <td><a href="salesAdd.php?deleteid=<?php echo htmlspecialchars($row['temp_id']); ?>" class="delete_item"><i class="fa fa-trash-o fa-2x text-error"></i></a></td>
                                ...
                                <td class="text text-warning sales_stock" id="reg_item_stock">
                                    <form action="salesAdd.php" method="post" class="line_item_form" autocomplete="off">
        ...
        <div class="col-md-3 sale_register_rightbox">
            <ul class="list-group">
                <li class="list-group-item nopadding">
                    <form action="salesAdd.php" method="post" id="cancel_sale_form" autocomplete="off">
                ...
                <li class="list-group-item">
                    <form action="salesAdd.php" method="post" id="finish_sale_form" autocomplete="off">
    ...
    <script type="module">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const forms = document.querySelectorAll('#mode_form, #select_customer_form, #add_payment_form, .line_item_form, #discount_all_form, #add_item_form, #finish_sale_form, #cancel_sale_form');
            ...
            document.getElementById('suspend_sale_button')?.addEventListener('click', () => {
                if (confirmAction('Are you sure you want to suspend this sale?')) {
                    fetch('salesAdd.php?suspend=suspend').then(res => res.text()).then(text => {
                        document.getElementById('register_container').innerHTML = text;
                    });
                }
            });
            ...
            // Delete items
            document.querySelectorAll('.delete_item, .delete_payment, #delete_customer').forEach(link => {
