    ...
    try {
        ...
        foreach ($currentLookup as $staffId => $currentEmp) {
            if (isset($snapshotLookup[$staffId])) {
                ...
                // Check allowance (gross) changes
                $snapshotGross = isset($snapshotEmp['gross_allowance']) ? (float)$snapshotEmp['gross_allowance'] : 0.0;
                $currentGross = isset($currentEmp['gross_allowance']) ? (float)$currentEmp['gross_allowance'] : 0.0;
                if ($currentGross !== $snapshotGross) {
                    $delta = $currentGross - $snapshotGross;
                    $deltaSign = $delta >= 0 ? '+' : '-';
                    $deltaAbs = abs($delta);
                    $employeeName = $currentEmp['NAME'] ?? $snapshotEmp['name'] ?? 'Unknown';
                    $description = "Allowance changed for {$employeeName}: ₦" . number_format($snapshotGross) . " → ₦" . number_format($currentGross) . " (Δ {$deltaSign}₦" . number_format($deltaAbs) . ")";

                    $query = "INSERT INTO abeokuta_change_log 
                             (submission_id, staff_id, change_type, old_value, new_value, change_description, detected_date) 
                             VALUES (?, ?, 'allowance_change', ?, ?, ?, CURDATE())";
                    $stmt = $conn->prepare($query);
                    $stmt->execute([
                        $submissionId,
                        $staffId,
        ...
        <div class="flex-1 p-6">
            <div class="container mx-auto">
                <div class="mb-8">
                    ...
                    <p class="text-gray-600">Track gross salary submissions with detailed snapshots and change history
                    </p>
                </div>
                ...
                <div class="bg-white rounded-xl shadow p-6 mb-6">
                    ...
                    <div class="grid md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div
                            ...
                            <div
                                class="text-sm text-<?php echo $varianceData['variance'] >= 0 ? 'green' : 'red'; ?>-600">
                                Variance</div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b">
                                    <td class="py-2 px-4">
                                        <?php echo date('M d, Y', strtotime($change['detected_date'])); ?>
                                    </td>
                                    <td class="py-2 px-4 font-mono"><?php echo htmlspecialchars($change['staff_id']); ?>
                                    </td>
                                    <td class="py-2 px-4">
                                        <?php 
                                        // Extract name from description or old_value/new_value JSON
                                        $name = 'Unknown';
                                        if ($change['change_type'] === 'new_employee' && $change['new_value']) {
                                            $data = json_decode($change['new_value'], true);
                                            $name = $data['NAME'] ?? 'Unknown';
                                        } elseif ($change['change_type'] === 'departed_employee' && $change['old_value']) {
                                            $data = json_decode($change['old_value'], true);
                                            $name = $data['name'] ?? 'Unknown';
                                        } elseif ($change['change_type'] === 'status_change') {
                                            // Status change: "Status changed: John Doe (Grade: 10/05)"
                                            if (preg_match('/Status changed: ([^(]+) \(/', $change['change_description'], $matches)) {
                                                $name = trim($matches[1]);
                                            }
                                        } elseif ($change['change_type'] === 'promotion') {
                                            // Promotion: "Promotion: John Doe (Grade: 10/05) → Jane Smith (Grade: 11/03)"
                                            if (preg_match('/Promotion: ([^(]+) \(/', $change['change_description'], $matches)) {
                                                $name = trim($matches[1]);
                                            }
                                        } elseif ($change['change_type'] === 'allowance_change') {
                                            // Allowance change: "Allowance changed for John Doe: ₦1,383,064 → ₦691,432 (Δ -₦691,632)"
                                            if (preg_match('/Allowance changed for ([^:]+):/', $change['change_description'], $matches)) {
                                                $name = trim($matches[1]);
                                            } else {
                                                $name = 'Employee #' . $change['staff_id'];
                                            }
                                        }
                                    ...
                                    <td class="py-2 px-4"><?php echo htmlspecialchars($change['change_description']); ?>
                                    </td>
                                    <td class="py-2 px-4"><?php echo htmlspecialchars($change['old_value'] ?? '—'); ?>
                                    </td>
                                    <td class="py-2 px-4"><?php echo htmlspecialchars($change['new_value'] ?? '—'); ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
