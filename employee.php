    ...
    <main class="flex-1 px-2 md:px-8 py-4 flex flex-col">
        <div class="w-full max-w-5xl mx-auto flex-1 flex flex-col">
            ...
            <?php if (isset($_SESSION['msg'])): ?>
                <div class="mb-4">
                    <div class="rounded px-4 py-3 shadow text-white 
                ...
                <table class="min-w-full text-sm">
                    ...
                    <?php foreach ($employees as $emp): ?>
                        <tr class="border-b hover:bg-blue-50">
                            <td class="py-1 px-2 font-mono text-blue-900"><?php echo htmlspecialchars($emp['staff_id']); ?></td>
                            ...
                            <td class="py-1 px-2"><?php echo htmlspecialchars($emp['GRADE'].'/'.$emp['STEP']); ?></td>
                            <td class="py-1 px-2"><?php echo htmlspecialchars($emp['PFANAME'] ?? ''); ?></td>
                            <td class="py-1 px-2"><?php echo htmlspecialchars($emp['BNAME'].'-'.$emp['ACCTNO']); ?></td>
                            <td class="py-1 px-2">
                                <?php
                            ...
                            <td class="py-1 px-2">
                                <div class="flex gap-2">
                                    <a href="javascript:void(0);" onclick="showEmployeeModal('<?php echo $emp['staff_id']; ?>')"
                                        class="text-blue-600 hover:text-blue-900" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="javascript:void(0);" onclick="showEditEmployeeModal('<?php echo $emp['staff_id']; ?>')"
                  ...
                  <?php foreach ($employees as $emp): ?>
                    <div class="bg-white rounded-xl shadow p-4 flex flex-col gap-2 w-full">
                      <div class="flex justify-between items-center">
                        <div class="font-bold text-blue-900 text-base"><?php echo htmlspecialchars($emp['NAME']); ?></div>
                      ...
                      <div class="flex gap-2 text-sm">
                        ...
                        <span class="font-mono text-blue-800"><?php echo htmlspecialchars($emp['staff_id']); ?></span>
                      </div>
                      <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-sm">
                        <div><span class="text-gray-600">Date:</span> <span><?php echo htmlspecialchars($emp['EMPDATE'] ?? ''); ?></span></div>
                        <div><span class="text-gray-600">Dept:</span> <span><?php echo htmlspecialchars($emp['dept'] ?? ''); ?></span></div>
                        <div><span class="text-gray-600">Grade/Step:</span> <span><?php echo htmlspecialchars($emp['GRADE'] ?? ''.'/'.$emp['STEP']); ?></span></div>
                        <div><span class="text-gray-600">PFA:</span> <span><?php echo htmlspecialchars($emp['PFANAME'] ?? ''); ?></span></div>
                        <div><span class="text-gray-600">Bank/No:</span> <span><?php echo htmlspecialchars($emp['BNAME'] ?? ''.'-'.$emp['ACCTNO'] ?? ''); ?></span></div>
...
<div id="editEmployeeModal"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-2 p-0 overflow-hidden relative max-h-[90dvh] overflow-y-auto">
        <div class="flex items-center justify-between px-5 py-4 bg-yellow-600 text-white">
            ...
            <button onclick="$('#editEmployeeModal').addClass('hidden')" class="text-white hover:text-yellow-300 text-2xl leading-3">&times;</button>
        </div>
        <form method="post" action="classes/controller.php?act=updateEmp" class="p-6 space-y-5" id="edit_employee_form" autocomplete="off">
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="$('#editEmployeeModal').addClass('hidden')" class="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
                <button type="submit" class="bg-yellow-600 text-white px-4 py-2 rounded hover:bg-yellow-700">Save Changes</button>
            </div>
        </form>
    </div>
</div>
...
<div id="addEmployeeModal"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-2 p-0 overflow-hidden relative max-h-[90dvh] overflow-y-auto">
        <div class="flex items-center justify-between px-5 py-4 bg-blue-700 text-white">
            ...
            <button onclick="$('#addEmployeeModal').addClass('hidden')" class="text-white hover:text-blue-300 text-2xl leading-3">&times;</button>
        </div>
        <form method="post" action="classes/controller.php?act=addNewEmp" class="p-6 space-y-5" id="employee_form" autocomplete="off">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="namee" required class="w-full px-3 py-2 border rounded" placeholder="e.g. John Doe">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="acct_no" required pattern="\d{10}" maxlength="10" class="w-full px-3 py-2 border rounded" placeholder="Account Number">
                </div>
                <div>
                ...
                <div>
                    ...
                    $final = $payp->fetch();
                    ?>
                    <input type="text" readonly name="emp_no" class="w-full px-3 py-2 border rounded bg-gray-100" required value="<?php echo intval($final['nextNo']) + 1 ?>">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="designation" required class="w-full px-3 py-2 border rounded" placeholder="Post">
                </div>
                <div>
                    <label class="block font-semibold text-sm mb-1">Grade</label>
                    <input type="text" name="grade" maxlength="3" required class="w-full px-3 py-2 border rounded" placeholder="Grade">
                </div>
                <div>
                    <label class="block font-semibold text-sm mb-1">Step</label>
                    <input type="text" name="gradestep" maxlength="2" required class="w-full px-3 py-2 border rounded" placeholder="Step">
                </div>
            </div>
            <!-- Radios -->
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="flex gap-4 mt-1">
                        <label class="inline-flex items-center"><input type="radio" name="callType" value="0" checked class="mr-1"> None</label>
                        <label class="inline-flex items-center"><input type="radio" name="callType" value="1" class="mr-1"> Doctor</label>
                        <label class="inline-flex items-center"><input type="radio" name="callType" value="2" class="mr-1"> Others</label>
                        <label class="inline-flex items-center"><input type="radio" name="callType" value="3" class="mr-1"> Nurse</label>
                    </div>
                </div>
                <div>
                    <label class="block font-semibold text-sm mb-1">Hazard Type:</label>
                    <div class="flex gap-4 mt-1">
                        <label class="inline-flex items-center"><input type="radio" name="hazardType" value="1" class="mr-1"> Clinical</label>
                        <label class="inline-flex items-center"><input type="radio" name="hazardType" value="2" class="mr-1"> Non-clinical</label>
                    </div>
                </div>
            </div>
            <!-- Actions -->
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="$('#addEmployeeModal').addClass('hidden')" class="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
                <button type="submit" name="addemp" class="bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-900">Add Employee</button>
            </div>
        </form>
    </div>
</div>
...
<div id="deactivateModal"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-2 p-0 overflow-hidden relative">
        <div class="flex items-center justify-between px-5 py-4 bg-blue-700 text-white">
            ...
            <button onclick="$('#deactivateModal').addClass('hidden')" class="text-white hover:text-blue-300 text-2xl leading-3">&times;</button>
        </div>
        <form id="deactivateEmpForm" method="post" action="classes/controller.php?act=deactivateEmployee">
            ...
            <div class="flex justify-end gap-2 px-5 pb-4">
                <button type="button" onclick="$('#deactivateModal').addClass('hidden')" class="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
                <button type="submit" class="bg-red-700 text-white px-4 py-2 rounded hover:bg-red-900">Update Status</button>
            </div>
        </form>
    </div>
</div>
...
<div id="employeeViewModal"
    class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 transition-all duration-150 ease-in-out hidden">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-2 p-0 overflow-hidden relative max-h-[90dvh] overflow-y-auto">
        <div class="flex items-center justify-between px-5 py-4 bg-blue-700 text-white">
            <div class="font-bold text-lg flex items-center gap-2">
                <i class="fas fa-id-card"></i> Employee Details
            </div>
            <button onclick="$('#employeeViewModal').addClass('hidden')" class="text-white hover:text-blue-300 text-2xl leading-3">&times;</button>
        </div>
...
function showEditEmployeeModal(staff_id) {
    ...
    $('#editEmployeeFields').html('<div class="text-center text-gray-400 py-10"><i class="fas fa-spinner fa-spin text-2xl"></i> Loading...</div>');
    // Fetch employee data via AJAX
    $.get('employee_view.php', {id: staff_id, edit: 1}, function(data) {
        if (data.error) {
            $('#editEmployeeFields').html('<div class="text-center text-red-500 p-10">'+data.error+'</div>');
            return;
        }
        // Build fields
        $('#editEmployeeFields').html(`
            ...
            <!-- ...add radios for callType/hazardType if needed -->
        `);
    }, 'json');
}
...
$('#employee_form').on('submit', function(e){
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.success) {
                $msg.html('<span class="text-green-600"><i class="fas fa-check-circle"></i> Employee added successfully!</span>');
                setTimeout(function(){
                ...
                $msg.html('<span class="text-red-600"><i class="fas fa-exclamation-triangle"></i> '+(res.error||'Unable to add employee')+'</span>');
            }
        },
        error: function(xhr) {
            $msg.html('<span class="text-red-600"><i class="fas fa-exclamation-triangle"></i> Server error. Please try again.</span>');
        }
    });
});
...
function showDeactivateModal(staff_id, name, currentStatus) {
    ...
    $('#deactivateModalBody').html('<div class="py-6 text-center text-gray-500"><i class="fas fa-spinner fa-spin text-2xl"></i> Loading...</div>');
    $.get('get_staff_status.php', function(statusList) {
        ...
        statusList.forEach(function(stat) {
            options += `<option value="${stat.STATUSCD}"${stat.STATUSCD===currentStatus?' selected':''}>${stat.STATUS}</option>`;
        });
...
function showEmployeeModal(id) {
    ...
    $('#employeeViewModalBody').html('<div class="text-center text-gray-400 py-10"><i class="fas fa-spinner fa-spin text-2xl"></i> Loading...</div>');
    $.get('edit_employee.php', {id}, function(data) {
        if (data.error) {
            $('#employeeViewModalBody').html('<div class="text-center text-red-500 p-10">'+data.error+'</div>');
            return;
        }
...
function statusMap(s) {
    return {A:'Active', D:'Dismissed', T:'Terminated', R:'Resigned', S:'Suspended'}[s]||'Unknown';
}
function callMap(c) {
    return {0:'None', 1:'Doctor', 2:'Others', 3:'Nurse'}[c]||'—';
}
function hazardMap(h) {
    return {1:'Clinical', 2:'Non-clinical'}[h]||'—';
}
