...
switch ($act) {
		...
		try {
			...
			if ($existtrans) {
				...
				if ($earningamount > -1) {
					...
					$conn->prepare($query)->execute(array($currentempl, $edcode, $earningamount, $recordtime, $_SESSION['SESS_MEMBER_ID'], '1'));
					$_SESSION['msg'] = $msg = "Earning successfully saved";
					$_SESSION['alertcolor'] = $type = "success";
					$source = $_SERVER['HTTP_REFERER'];
					//redirect($msg, $type, $source);					
	...
	case 'addperiod':
		...
		try {
			...
			if ($row = $query->fetch()) {
				...
				$conn->prepare($query)->execute(array($insertPayId, $periodname, $periodyear, '0', '0'));
	...
	case 'addNewEmp':
		...
		try {
			...
			$query = 'INSERT INTO employee (employee.staff_id,employee.`NAME`, employee.EMPDATE, employee.DEPTCD, employee.POST, employee.GRADE, employee.STEP, employee.ACCTNO, employee.BCODE, employee.CALLTYPE, employee.STATUSCD, employee.PFACODE, employee.PFAACCTNO,userID,editTime) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';

			$conn->prepare($query)->execute(array($emp_no, $namee, $doe, $dept, $designation, $grade, $gradestep, $acct_no, $bank, $callType, 'A', $pfa, $rsa_pin, $_SESSION['SESS_MEMBER_ID'], $recordtime,));
	...
	case 'updateEmp':
		...
		try {
			...
			$query = 'UPDATE employee SET employee.`NAME` = ? ,employee.POST = ?, employee.DEPTCD = ?, employee.CALLTYPE = ?, employee.GRADE = ?, employee.STEP = ?, employee.EMPDATE = ?, employee.BCODE = ?, employee.ACCTNO = ?, employee.PFACODE = ?, employee.PFAACCTNO = ? WHERE staff_id = ?';
			...
			$conn->prepare($query)->execute(array($namee, $post, $dept,  $callType, $grade, $gradestep, $doe, $bank, $acct_no, $pfa, $rsa_pin, $emp_no));
