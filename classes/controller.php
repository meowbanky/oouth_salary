...
switch ($act) {
    ...
    case 'login':
        ...
        $uname = filter_var((filter_var($_POST['username'])));
        ...
        try {
            $query = $conn->prepare('SELECT employee.name, employee.EMAIL,username.username,username.role_id, username.`password`, username.role, username.staff_id FROM username
...
//                error_log("am here: ".$row['password']);
                ...
                $payp = $conn->prepare('SELECT periodId, description, periodYear FROM payperiods WHERE active = ? ORDER BY periodId ASC LIMIT 1');
                ...
                //$page = "../../dashboard.php";
                $_SESSION['msg'] = $msg = "Welcome " . $_SESSION['SESS_FIRST_NAME'];
...
//        break;
    ...
    case 'addperiod':
        try {
            ...
            $query = $conn->prepare('SELECT 1 FROM payperiods WHERE description = ? AND periodYear = ?');
            $query->execute([$periodname, $periodyear]);
            ...
            // Get last period ID
            $payp = $conn->prepare('SELECT periodId FROM payperiods ORDER BY id DESC LIMIT 1');
            $payp->execute();
            $final = $payp->fetch();
            $insertPayId = intval($final['periodId']) + 1;

            // Insert new period
            $conn->prepare('INSERT INTO payperiods (periodId, description, periodYear, active, payrollRun) VALUES (?,?,?,?,?)')
                ->execute([$insertPayId, $periodname, $periodyear, '1', '0']);

            // Update previous period
            $conn->prepare('UPDATE payperiods SET active = ? WHERE periodId = ?')
                ->execute([0, $final['periodId']]);

            // Clear allowances
            $conn->prepare('DELETE FROM allow_deduc WHERE allow_id = ?')
                ->execute([3]);

            // Get period details
            $sql_periodDetails = "SELECT periodId, CONCAT(description,' - ', periodYear) as description 
                    FROM payperiods WHERE active = 0 
                    ORDER BY periodId DESC LIMIT 1";
            $stmt_periodDetails = $conn->prepare($sql_periodDetails);
            $stmt_periodDetails->execute();
            $row_periodDetails = $stmt_periodDetails->fetch();
            $periodDetails = $row_periodDetails['description'];

            // Send notifications
            sendNotificationToAll($conn,
                $periodDetails . " Payslip",
                "Your Payslip for the period " . $periodDetails . " is now available for download"
            );
            ...
            $conn->rollBack();
            $_SESSION['msg'] = "Error creating period: " . $e->getMessage();
            $_SESSION['alertcolor'] = "danger";
        }
