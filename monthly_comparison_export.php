...
fputcsv($output, ['New Employees', $reportData['summary']['new_employees']]);
fputcsv($output, ['Departed Employees', $reportData['summary']['departed_employees']]);
fputcsv($output, ['Status Changes', $reportData['summary']['status_changes']]);
fputcsv($output, ['Promotions/Demotions', $reportData['summary']['promotions']]);
fputcsv($output, ['Allowance Changes', $reportData['summary']['allowance_changes']]);
fputcsv($output, ['']);

// Write financial impact
fputcsv($output, ['ALLOWANCE IMPACT SUMMARY']);
fputcsv($output, ['Allowance Increases', '₦' . number_format($reportData['summary']['total_allowance_increase'])]);
fputcsv($output, ['Allowance Decreases', '₦' . number_format($reportData['summary']['total_allowance_decrease'])]);
fputcsv($output, ['Net Allowance Change', '₦' . number_format($reportData['summary']['net_allowance_change'])]);
fputcsv($output, ['GROSS DIFFERENCE', '₦' . number_format($reportData['summary']['gross_difference'])]);
fputcsv($output, ['']);
    ...
    if (!empty($allowanceIncreases)) {
        ...
        foreach ($allowanceIncreases as $change) {
            fputcsv($output, [
                $change['staff_id'],
                $change['NAME'],
                number_format($change['previous_total']),
                number_format($change['current_total']),
                number_format($change['total_difference'])
            ]);
        }
        fputcsv($output, ['']);
        fputcsv($output, ['TOTAL INCREASES', '₦' . number_format($totalIncrease)]);
        fputcsv($output, ['']);
    }
    ...
    if (!empty($allowanceDecreases)) {
        ...
        foreach ($allowanceDecreases as $change) {
            fputcsv($output, [
                ...
                number_format($change['previous_total']),
                number_format($change['current_total']),
                number_format(abs($change['total_difference']))
            ]);
        }
        fputcsv($output, ['']);
        fputcsv($output, ['TOTAL DECREASES', '₦' . number_format($totalDecrease)]);
        fputcsv($output, ['']);
    }
}
