...
try {
    ...
    $Data['StaffNo']  = '';
    $Data['Name'] = 'TOTAL';
    $Data['Amount']  = number_format($sumTotal);
    if ($deduction == 87 || $deduction == 85) {
        $loan = retrieveLoanStatus($link['staff_id'], $deduction);
        $repayment = retrieveLoanBalanceStatus($link['staff_id'], $deduction, $period);
        $Data['Balance'] = '';
...
$tempfilepath = $deduction_text . '.xlsx';
...
if ($existtrans) {
    ...
    $tempfilepath = $deduction_text;
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment; filename="' . $tempfilepath . '.xlsx"');
    header('Cache-Control: max-age=0');

    // Read the Excel file content
    readfile($tempfilepath . ".xlsx");
    ...
    if (!$mail->send()) {
        ...
        if (file_exists($tempfilepath . ".xlsx")) {
            $status  = unlink($tempfilepath . ".xlsx");
        } else {
